...
class CuroboPlanner(MotionPlannerBase):
    ...
    def __init__(
        ...
        self.motion_gen.warmup(enable_graph=True, warmup_js_trajopt=self.config.warmup_js_trajopt)
    ...
    def get_object_pose(self, object_name: str) -> Pose | None:
        ...
        Args:
            object_name: Short object name used in Isaac Lab scene (e.g., "object_1")
    ...
    def _update_object_in_world_model(
        ...
        Args:
            ...
            object_name: Short object name from Isaac Lab (e.g., "object_1")
    ...
    def _attach_object(self, object_name: str, object_path: str, env_id: int) -> bool:
        ...
        Args:
            object_name: Short name of the object to attach (e.g., "object_2")
    ...
    def get_attached_objects(self) -> list[str]:
        ...
        Returns:
            List of attached object names (e.g., ["object_1", "object_2"])"""
    ...
    def _plan_to_contact_pose(
        ...
        try:
            ...
            if result.success.item():
                self._store_plan_from_result(result)
    ...
    def _store_plan_from_result(self, result: Any) -> None:
        """Extract the trajectory from a MotionGenResult and store it as the current plan.

        When ``config.use_interpolated_plan`` is set, the time-uniform interpolated plan
        (one waypoint every ``interpolation_dt`` seconds) is preferred so that executing one
        waypoint per control step reproduces the planned velocity profile. Otherwise the raw
        trajopt ``optimized_plan`` is used (legacy behavior — its waypoints carry a variable
        ``optimized_dt`` time base that a fixed-rate executor does not respect).

        Args:
            result: A successful MotionGenResult from plan_single / plan_single_js.
        """
        interpolated = result.get_interpolated_plan() if self.config.use_interpolated_plan else None
        if interpolated is not None and len(interpolated.position) != 0:
            self._current_plan = interpolated
            self.logger.debug(
                f"Using interpolated plan with {len(self._current_plan.position)} waypoints"
                f" (dt={self.config.interpolation_dt:.4f}s)"
            )
        elif result.optimized_plan is not None and len(result.optimized_plan.position) != 0:
        ...
        common_js_names: list[str] = [x for x in self.robot.data.joint_names if x in self._current_plan.joint_names]
        self._current_plan = self._current_plan.get_ordered_joint_state(common_js_names)
        self._plan_index = 0

    def plan_to_joint_target(self, target_joint_pos: torch.Tensor, joint_names: list[str]) -> bool:
        """Plan a collision-free joint-space motion to a target configuration.

        Used to return the robot to a known configuration (e.g., its home pose) between
        manipulation phases. The world model is synchronized, the gripper is treated as open,
        and any attached objects are detached before planning.

        Args:
            target_joint_pos: Target joint positions, one value per entry in ``joint_names``.
            joint_names: Joint names corresponding to ``target_joint_pos``. May contain more
                joints than cuRobo actuates (e.g., finger joints) — extras are ignored.

        Returns:
            True if planning succeeded and the trajectory is stored as the current plan.
        """
        self.reset_plan()
        self.update_world()
        self._set_gripper_state(False)
        if self.get_attached_objects():
            self._detach_objects()

        start_state: JointState = self._get_current_joint_state_for_curobo()

        goal_pos = self._to_curobo_device(target_joint_pos.detach().float().unsqueeze(0))
        goal_state = JointState(
            position=goal_pos,
            velocity=torch.zeros_like(goal_pos),
            acceleration=torch.zeros_like(goal_pos),
            joint_names=list(joint_names),
            tensor_args=self.tensor_args,
        ).get_ordered_joint_state(self.motion_gen.kinematics.joint_names)

        try:
            result: Any = self.motion_gen.plan_single_js(start_state, goal_state, self.plan_config)
        except Exception as e:
            self.logger.warning(f"Joint-space planning error: {e}")
            return False

        if not result.success.item():
            self.logger.warning(f"Joint-space planning failed: {result.status}")
            return False

        self._store_plan_from_result(result)
        self.logger.debug(f"Joint-space planning succeeded with {len(self._current_plan.position)} waypoints")
        return True
    ...
    def _check_object_grasped(self, gripper_pos: torch.Tensor, object_name: str) -> bool:
        ...
        Args:
            ...
            object_name: Name of object to check (e.g., "object_1")
