...
class CuroboPlannerCfg:
    ...
    use_interpolated_plan: bool = False
    """Prefer the time-uniform interpolated plan (sampled at ``interpolation_dt``) over the raw
    trajopt ``optimized_plan``. Set ``interpolation_dt`` to the executor's control period so that
    consuming one waypoint per control step reproduces the planned timing."""

    warmup_js_trajopt: bool = False
    """Also warm up the joint-space trajectory optimizer at startup. Required for reliable
    ``plan_to_joint_target()`` calls (e.g., home-return motions); adds a little warmup time."""
    ...
    @classmethod
    def franka_pack_object_bin_config(cls) -> "CuroboPlannerCfg":
        """Create configuration for Franka packing object in a bin."""
    ...
    @classmethod
    def franka_pack_object_config(cls) -> "CuroboPlannerCfg":
        """Create configuration for Franka packing object."""
        config = cls.franka_config()
    ...
    def from_task_name(cls, task_name: str) -> "CuroboPlannerCfg":
        ...
        Args:
            task_name: Task name (e.g., "Isaac-Pack-Object-Bin-Franka-v0")
        ...
        if "pack-object-bin" in task_lower:
            return cls.franka_pack_object_bin_config()
        elif "pack-object" in task_lower:
            return cls.franka_pack_object_config()
        else:
