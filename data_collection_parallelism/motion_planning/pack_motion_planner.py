...
from data_collection_parallelism.grasp_client.grasp_result import GraspResult
...
class PackMotionPlanner:
    ...
    def __init__(
        ...
        """
        Args:
            num_trajopt_seeds: Override CuroboPlannerCfg's default (12). Measured 2026-07-23:
                each seed count roughly scales this planner instance's VRAM footprint (12 seeds
                -> ~3.6GB, 4 seeds -> ~1.6GB per instance) since seeds are parallel GPU-batched
                optimization attempts. Lowering this is a real tradeoff, NOT a free win — fewer
                seeds means fewer candidate starting points per planning call, which can lower
                the pick/place planning success rate (this task's grasp poses are often marginal;
                see max_planning_attempts below). None keeps the factory default (12) — no
                behavior change unless explicitly overridden.
            num_graph_seeds: Same tradeoff as num_trajopt_seeds, for the graph (RRT) search phase.
        """
        self.env = env
        self.robot = robot
        self.env_id = env_id
        ...
        if num_trajopt_seeds is not None:
            cfg.num_trajopt_seeds = num_trajopt_seeds
        if num_graph_seeds is not None:
            cfg.num_graph_seeds = num_graph_seeds
